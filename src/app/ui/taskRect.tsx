...
export default function TaskRect({
    ...
    switchTaskIndices: (task: Task, oldIndex: number, newIndex: number, dynamicSwitchMode: boolean) => void,
    color: string,
    visible: boolean,
}) {
    ...
    const grabTask = async (evt: React.MouseEvent) => {
        ...
        if (thisDiv) {
            ...
            const reorderDynamic = (oldIndex: number, newIndex: number) => {
                switchTaskIndices(task, oldIndex, newIndex, true);
            ...
            const stopMove = () => {
                ...
                switchTaskIndices(task, task.index, newIndex, false);

            };

            const calculateIndexDynamic = () => {
                let newIndex = calculateNewIndex(draggableDiv.getCurrentPosition(progBar, thisDiv)!)
                console.log(task.index, "--", newIndex);
                if (newIndex !== task.index) reorderDynamic(task.index, newIndex);
            }

            thisDiv.addEventListener("mousemove", calculateIndexDynamic);
